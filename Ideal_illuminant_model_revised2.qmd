...
mi1.side.contr <- contrast(mi1.side.EM, list(
    B1_D2 = c(-1, 0, 0, 1),
    D1_B2 = c(0, 1, -1, 0),
    B1_D1 = c(1, 0, -1, 0),
    B2_D2 = c(0, 1, 0, -1)
...
forplot <- summary(mi1.side.contr)
plottext_B1D1 <- paste0(
    round(forplot[3, "estimate"], 2), "(",
    round(forplot[3, "SE"], 2), ")"
)
plottext_B2D2 <- paste0(
    round(forplot[4, "estimate"], 2), "(",
    round(forplot[4, "SE"], 2), ")"
)
...
psidetheta <- ggplot(plot.mi1.side, aes(x = TypeAge, y = the.emmean, ymin = lower.CL, ymax = upper.CL, color = group)) +
    ...
    # geom_text(label = plottext_B1D1, x = 1.5, y = 0.225, color = "gray40") +
    # geom_text(label = plottext_B2D2, x = 3.5, y = 0.225, color = "gray40") +
    theme_bw() +
